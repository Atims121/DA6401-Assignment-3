...
parser.add_argument('-wp', '--wandb_project', type=str, default='DA6401-Assignment-3', help='Project name used to track experiments in Weights & Biases dashboard')
...
parser.add_argument('-d', '--dataset', type=str, default='dakshina_dataset_v1.0', choices=["dakshina_dataset_v1.0"], help='Dataset choices: ["dakshina_dataset_v1.0"]')
parser.add_argument('-l', '--language', type=str, default='hi', help='Language choices: ["te", "mal", "ta"]')
...
# Load the dataset contraining words transliterated from one script to another

dataset_name = args.dataset
language = args.language
list_files = os.listdir(f'{dataset_name}/{language}')
path = f'{dataset_name}/{language}/lexicons/'


# Load the there data files 
X_test, y_test = load_dataset_csv(f'{path}/{language}.translit.sampled.test.tsv')
X_train, y_train = load_dataset_csv(f'{path}/{language}.translit.sampled.train.tsv')
X_val, y_val = load_dataset_csv(f'{path}/{language}.translit.sampled.dev.tsv')
...
if not args.wandb_sweepid is None:
    ...
    def hyper_config_run(config=None):
        with wandb.init(config=config, reinit=True) as run:  # type: ignore
            ...
            if attention:
                ...
                encoder = Encoder(input_size=latin_script.vocab_size, hidden_size=hidden_size, RNN=RNN_ENC,inp_emmbed_size=input_embedding_size, num_layers=num_encoder_layers, dropout_p=dropout_encoder).to(device)
                ...
                encoder = Encoder(input_size=latin_script.vocab_size, hidden_size=hidden_size, RNN=RNN, inp_emmbed_size=inp_embed_size,num_layers=num_encoder_layers, dropout_p=encoder_dropout_p).to(device)
                decoder = Decoder(hidden_size=hidden_size, max_length=MAX_LENGTH, RNN=RNN, output_size=local_script.vocab_size, dropout_p=decoder_dropout_p, num_decoder_layers=num_decoder_layers, device=device).to(device)
    ...
    if attention:
        ...
        encoder = Encoder(input_size=latin_script.vocab_size, hidden_size=hidden_size, inp_emmbed_size=inp_embed_size, num_layers=num_encoder_layers, dropout_p=encoder_dropout_p).to(device)
        decoder = Decoder(hidden_size=hidden_size, max_length=MAX_LENGTH, output_size=local_script.vocab_size, dropout_p=decoder_dropout_p, num_decoder_layers=num_decoder_layers, device=device).to(device)
