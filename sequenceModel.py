...
class Encoder(torch.nn.Module): 
    ...
    def __init__(self, input_size, hidden_size, num_layers=2, inp_emmbed_size=16, RNN=torch.nn.GRU, dropout_p=0.1):
    ...
    def forward(self, input_tensor):
        embedded = self.dropout(self.embedding(input_tensor))  # [batch, seq_len, embed_size]

        # Let PyTorch initialize hidden state internally to zeros
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, encoder_outputs, encoder_hidden, target_tensor=None): 
        ...
        decoder_input = torch.empty(batch_size, 1, dtype=torch.long, device=self.device).fill_(0)  # <SOS> token
    ...
    def forward_step(self, input_tensor: torch.Tensor, hidden: torch.Tensor):
        ...
        embedded = self.dropout(self.embedding(input_tensor))  # [batch, seq_len, embed_dim]
        embedded = torch.nn.functional.relu(embedded)

        # No bidirectional adjustment needed here, so just use the hidden as is

        output, hidden = self.rnn(embedded, hidden)  # RNN step
        output = self.out(output)  # Project to output vocab size
